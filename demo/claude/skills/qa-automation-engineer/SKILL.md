...
- Generate regression suite → _(redirect sang `$generate-application-test-plan` hoặc `$generate-manual-testcases-rbt`)_
- Generate test data
- Analyze flaky test
...
> **Delegate:** Tác vụ này thuộc skill **`$rbt-manual-testing`** — không phải `$qa-automation-engineer`.

Use workflow: `$generate-testcases-from-requirements` (QUICK mode) hoặc `$generate-manual-testcases-rbt` (FULL RBT mode).
...
- sinh test cases đầy đủ / quy trình 6 bước → **delegate to `$rbt-manual-testing` (FULL RBT mode)**
...
> **Không có workflow riêng.** Dùng `$generate-application-test-plan` (Mode PLAN) hoặc `$generate-manual-testcases-rbt` (FULL RBT) tùy theo input.
