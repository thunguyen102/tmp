...
- Cần sinh automation code → dùng `$qa-automation-engineer`
- Cần inspect DOM / sinh locator → dùng `$ui-debug-agent` / `$smart-locator-agent`
- Chỉ cần sinh test data → dùng `$test-data-generator`
...
Kích hoạt khi:
- User dùng workflow `$generate-testcases-from-requirements`
...
Kích hoạt khi:
- User dùng workflow `$generate-manual-testcases-rbt`
...
> - **Luồng Codex skill:** Thực hiện theo hướng dẫn tổng quát bên dưới. Không cần đọc file `prompt.txt`.
> - **Luồng Copy-Paste:** QA team copy tuần tự từ `plans/manual/01_context_and_roleplay/prompt.txt` đến `plans/manual/06_template_mapping/prompt.txt` vào chat AI, từng bước một.
