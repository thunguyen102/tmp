...
try {
  ...
  await page.getByLabel('Vacancy Name').fill(uniqueName);
  await page.getByText('-- Select --', { exact: true }).first().click();
  await page.getByText('QA Engineer', { exact: true }).click();
  await page.getByLabel('Hiring Manager').fill('Sumana');
  await page.waitForTimeout(1000);
  await page.getByText('Sumana Testuser', { exact: true }).first().click();
  await page.getByRole('button', { name: 'Save' }).click();
  await page.waitForLoadState('networkidle', { timeout: 60000 }).catch(() => {});
  await page.screenshot({ path: path.join(outDir, 'save-success-labeled.png'), fullPage: true });
  fs.writeFileSync(path.join(outDir, 'save-success-labeled-body.txt'), await page.locator('body').innerText(), 'utf8');
  fs.writeFileSync(path.join(outDir, 'save-success-labeled-name.txt'), uniqueName, 'utf8');
} catch (error) {
  fs.writeFileSync(path.join(outDir, 'save-success-labeled-error.txt'), String(error?.stack || error), 'utf8');
} finally {
  await browser.close();
}
