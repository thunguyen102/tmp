...
2. Use `requirements-analysis.md` as the main input when it exists. Keep its ambiguity and risk codes traceable, but do not copy unresolved items mechanically. Treat a relevant URL embedded in any input artifact as supplied. For a reachable web target, invoke or extend `qa-explore-real-ui` when evidence is missing, stale, incomplete, conflicting, or can resolve an open item; reconcile the source artifacts before planning.
...
- Resolve discoverable gaps before writing the plan. Ask only for unavailable access/credentials, approval for risky actions, or non-observable business decisions.
- Keep an open question only after recording the attempted evidence path and why it remains unresolved.
- Whenever a relevant URL is supplied or current UI evidence is insufficient, require a real browser run through `qa-explore-real-ui`. Reuse current verified evidence from the same QA chain; do not ask the user to install or operate browser tooling.
- Do not label coverage as validated when `ui-evidence.md` is missing or `BLOCKED`; limit the plan to observed facts and expose blocked areas in entry criteria, risks, and open questions.
...
Continue to test cases only when the user requested the full chain or test cases. Otherwise report completion concisely without a routine handoff question.
