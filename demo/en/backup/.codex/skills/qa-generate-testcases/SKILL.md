...
1. Read and understand the supplied requirements and test plan. Use the existing files under `qa-automation/<feature-slug>/` as the main input when available. Treat a relevant URL embedded in any input artifact as supplied. For a reachable web target, invoke or extend `qa-explore-real-ui` when evidence is missing, stale, incomplete, conflicting, or can resolve an open item; reconcile all inputs before writing cases.
2. Run a final ambiguity-closure pass. Resolve every safely observable field rule, option, message, default, filter, action, and state behavior. Do not carry an open question from the plan into test cases merely because it already exists.
3. Identify applicable coverage groups:
   - Happy paths
   - Negative paths
   - Boundary cases
   - Edge cases
...
4. Apply suitable test design techniques:
   - Equivalence Partitioning (EP)
   - Boundary Value Analysis (BVA)
   - Decision Table for combined business rules
   - State Transition Testing for workflows and statuses
5. For forms and UI fields, create field-specific validation cases using [`references/field_validation_checklist.md`](./references/field_validation_checklist.md).
...
7. Save the cases as a standard Markdown table. If a case needs complex steps, use numbered steps with `<br>` inside the table cell or a detailed subsection linked to the TC ID.
...
- Do not create executable-looking steps or expected UI results from assumptions. Whenever a relevant URL is supplied or current UI evidence is insufficient, require real browser evidence through `qa-explore-real-ui`. Reuse current verified evidence from the same QA chain; do not ask the user to install tools or interact with the site.
- Ask only for unavailable access/credentials, approval for risky actions, or non-observable business intent. Human review is the final check, not a substitute for exploration.
- Keep an open question or automation blocker only after exhausting safe evidence paths and recording what was attempted.
- If real UI exploration is `BLOCKED`, generate cases only for verified behavior, label the set partial, and list every excluded unverified area under `Automation Notes`.
